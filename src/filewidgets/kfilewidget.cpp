...
public:
    explicit KFileWidgetPrivate(KFileWidget *qq)
        : q(qq),
          m_autoSelectExtChecked(false),
          m_keepLocation(false),
          m_hasView(false),
          m_hasDefaultFilter(false),
          m_inAccept(false),
          m_dummyAdded(false),
    ...
    // Not used anywhere
    // bool m_autoDirectoryFollowing : 1;

    bool m_hasDefaultFilter : 1; // necessary for the m_operationMode
    bool m_inAccept : 1; // true between beginning and end of accept()
    bool m_dummyAdded : 1; // if the dummy item has been added. This prevents the combo from having a
    // blank item added when loaded
    bool m_confirmOverwrite : 1;
...
#if QT_VERSION < QT_VERSION_CHECK(5, 15, 0)
    QStringList types = d->m_filterWidget->currentFilter().split(QLatin1Char(' '), QString::SkipEmptyParts); //QStringList::split(" ", d->m_filterWidget->currentFilter());
#else
...
void KFileWidgetPrivate::readViewConfig()
{
    ...
    // Not used anywhere
    // m_autoDirectoryFollowing = m_configGroup.readEntry(AutoDirectoryFollowing, DefaultDirectoryFollowing);
